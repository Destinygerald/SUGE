...
export async function fetchBlogContent(Id) {
  ...
  const response = await axios.get(`${URL}/blog/by-slug/${Id}`);
  return response.data;
}
...
export async function deleteBlogImage(id) {
  const response = await axios.delete(`${URL}/admin/delete-image/${id}`, {
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
    },
...
export function getCookie() {
  ...
  const authCookie = allCookies.find((item) =>
    item.includes("admin_auth_token")
  );
...
export async function add_image(imgData) {
  try {
    const response = await axios.post(`${URL}/admin/add-image`, imgData, {
      headers: {
        Accept: "application/json",
