...
function returnInput(pathname, i, id, handleImage, blogInfo) {
  if (
    ...
    !pathname.includes("create") &&
    i != 2 &&
    (blogInfo.template == 2 || blogInfo.template == 3)
  ) {
    return (
      <input
        type="file"
    ...
    !pathname.includes("create") &&
    i == 1 &&
    (blogInfo.template == 1 || blogInfo.template == 4)
  ) {
    return (
      <input
        type="file"
...
function BlogParagraph({
  ...
  useEffect(() => {
    if (
      ...
      typeof blogInfo?.content[i]?.img == "object"
    ) {
  ...
  return (
    <div className="admin-blog-paragraph">
      ...
      {i == 0 ? (
        <>
          <TextEditor name="content" value={cnt} onChange={textEditorChange} />
        </>
      ) : (
        <>
      ...
      {typeof image == "object" ? (
        <>
          <div className="paragraph-img">
            <img
              src=""
              className="paragraph-img-cnt"
              id={`paragraph-img-cnt-${i}`}
            />
          </div>

          <button className="paragraph-img-btn" onClick={removeImage}>
            Remove
          </button>
        </>
      ) : image &&
        ...
        typeof image != "object" &&
        image != `${URL}/blog/blog-image/` ? (
        <>
          <div className="paragraph-img">
            <img
              src={`${URL}/blog/blog-image/${image}`}
              className="paragraph-img-cnt"
            />
          </div>
...
export function CreateBlog({ msg, setMsg }) {
  ...
  async function deleteThisBlog() {
    ...
    try {
      ...
      for (const data of blogInfo?.content) {
        if (data.img) {
          const res = await deleteBlogImage(data?.img);
        }
      }
  ...
  async function editBlogCnt() {
    ...
    const previousData = await fetchBlogContent(id);
    ...
    for (let i = 0; i < 3; i++) {
      if (
        ...
        if (previousData?.data.content[i]?.img) {
        ...
        blogInfo.content[i].img &&
        typeof blogInfo.content[i].img == "string"
      ) {
        img_ids.push({
          id: i,
          img: blogInfo.content[i].img,
        });
      } else {
      }
    }
    ...
    setTimeout(async () => {
      const res = await editBlogs(id, reqData);
  ...
  function preview() {
    ...
    navigate("/admin/dashboard/blog/preview", {
      state: {
        history: pathname,
        type: pathname?.includes("/create") ? "create" : "edit",
      },
    });
  }
  ...
  return (
    <div className="create-blog">
      <div className="create-blog-hdr">
        ...
        <span>
          {pathname.split("/").includes("create") ? "Create" : "Edit"} Blog
        </span>
      </div>
      ...
      <div className="create-blog-main">
        <input
          ...
          name="meta_data_title"
          value={blogInfo?.meta_data_title}
          onChange={changeHandler}
          className="create-blog-main-input"
        />
        <input
          type="text"
            ...
            {blogInfo?.title || blogInfo?.readTime || blogInfo?.content[0]
              ? blogInfo?.content?.map((item, i) => (
                  <BlogParagraph
                    ...
                    cnt={item?.content}
                    image={item?.img}
                    changeHandler={(e) => paragraphChangeHandler(e, i)}
                    setBlogInfo={setBlogInfo}
                    blogInfo={blogInfo}
                    textEditorChange={(e) => textEditorChange(e, i)}
                  />
                ))
      ...
      <div className="create-blog-btn">
        {pathname.split("/").includes("create") ? (
          <button disabled={click} onClick={createBlog} className="submit-btn">
            Submit
          </button>
        ) : (
          <button disabled={click} onClick={editBlogCnt} className="submit-btn">
            Edit
          </button>
        )}
        ...
        <button className="add-p-btn" onClick={preview}>
          Preview
        </button>
      </div>
